...
guest, use the Augeas functions (see L</aug-init>).
...
L</case-sensitive-path> attempts to resolve the true case of
...
I<Note>:
Because of the above behaviour, L</case-sensitive-path> cannot
...
Most users should use L</cp-a> instead.  This command
...
I<This function is deprecated.>
In new code, use the L</get-hv> call instead.
...
(see: L</journal-set-data-threshold>,
L</journal-get-data-threshold>).
...
(eg. using L</journal-get-data>), and C<false> means you
...
The contents of the journal can be read using L</journal-next>
and L</journal-get>.

After you have finished using the journal, you should close the
handle by calling L</journal-close>.
...
(see L</glob-expand>).  In guestfish you can use the C<glob>
...
You can use L</vfs-uuid> to return the existing UUID
of a filesystem.

I<This function is deprecated.>
In new code, use the L</set-uuid> call instead.
...
I<This function is deprecated.>
In new code, use the L</set-hv> call instead.
