...
ACL on the file.  The ACL must contain the full Unix permissions
(eg. C<u::rwx,g::rx,o::rx>).

If you are specifying individual users or groups, then the mask field is
also required (eg. C<m::rwx>), followed by the C<u:I<ID>:...> and/or
C<g:I<ID>:...> field(s).  A full ACL string might therefore look like this:
...
B<この関数を使用しないでください!> ISO ファイルは通常読み込み専用のディスクイメージです。代わりに L</add-drive-ro>
を使用してください。

I<この関数は推奨されません。> 新しいコードでは、代わりに L</add-drive> を使用してください。
...
他のオプション引数は直接 L</add-drive-opts> に渡されます。

このコマンドは 1 つまたはそれ以上のオプション引数を持つ必要があります。 L</オプション引数> を参照してください。
...
その値が C<expr> を評価した結果である変数 C<name> を定義します。
...
成功した場合、C<expr> にあるノード数が返されます。または、C<expr> がノードセット以外のものであると評価された場合、C<0>
が返されます。
...
C<path> と関連づけられた値を検索します。C<path> がちょうど一つのノードと一致した場合、C<value> が返されます。
...
L</aug-init> にあるツリーを読み込みません。

=back

ハンドルを閉じるには、L</aug-close> を呼び出す必要があります。
...
L</available> / L</feature-available> on each member of the returned list.

See also L</available>, L</feature-available> and
...
標準入力/標準出力から読み込み/書き込みするには、ファイル名の代わりに C<-> を使用します。
...
C<device> と関連づけられている内部バッファーをフラッシュするようカーネルに指示します。
...
L</lvm-canonical-lvm-name> を使用して C</dev/VG/LV> に変換されます。
...
the remainder appended unmodified. For example, if the file
C<"/Windows/System32/netkvm.sys"> exists:
...
L</list-devices>, L</part-to-dev> 参照。
...
I<この関数は推奨されません。> 新しいコードでは、代わりに L</grep> を使用してください。
...
You also have to mark the partition as "active" (L</part-set-bootable>) and
a Master Boot Record must be installed (eg. using L</pwrite-device>) on the
first sector of the whole disk.  The SYSLINUX package comes with some
...
このコマンドを使用する前に L</launch> を呼び出す必要があります。
...
L</fstrim> either instead of, or after calling L</zero-free-space>.
...
I<この関数は推奨されません。> 新しいコードでは、代わりに L</vfs-uuid> を使用してください。
...
I<This function is deprecated.> In new code, use the L</get-hv> call
instead.
...
L<guestfs(3)> の SELINUX および L</setcon> に関するドキュメントを参照してください
...
L</inspect-get-format> が C<installer> (つまり、インストールディスク)
を返した場合、ライブイメージがディスクにおいて検知されると、これが真を返します。
...
L</inspect-get-format> が C<installer> (つまり、インストールディスク)
を返した場合、ディスクがあるセットの一部であると、これが真を返します。
...
I<This function is deprecated.> In new code, use the
L</inspect-list-applications2> call instead.
...
string).  Please note that C<attrval> is a byte array, I<not> a
\0-terminated C string.

The length of data may be truncated to the data threshold (see:
L</journal-set-data-threshold>, L</journal-get-data-threshold>).
...
See also L</journal-set-data-threshold>.
...
L</journal-get-data>), and C<false> means you have reached the end of the
journal.
...
The contents of the journal can be read using L</journal-next> and
L</journal-get>.
...
See also L</journal-get-data-threshold>.
...
I<この関数は推奨されません。> 新しいコードでは、代わりに L</shutdown> を使用してください。
...
L</add-drive-opts>, you can use this call to map between disk labels, and
raw block device and partition names (like C</dev/sda> and C</dev/sda1>).
...
L</is-lv>, L</canonical-device-name> 参照。
...
L</add-drive-opts> and similar calls).
...
I<この関数は推奨されません。> 新しいコードでは、代わりに L</mke2fs> を使用してください。
...
I<この関数は推奨されません。> 新しいコードでは、代わりに L</mkswap> を使用してください。
...
'mountable' strings returned by L</list-filesystems> or
...
これは追加されたブロックデバイス全体の数を返します。これは L</list-devices> により返されるデバイス数と同じです。

追加できるデバイスの最大数を確認するには L</max-disks> を呼び出しください。
...
L<guestfs(3)/guestfs_create_flags>, and L</parse-environment-list>.
...
L</part-to-partnum>, L</device-index> も参照してください。
...
Note that at the moment you must supply the "optional" C<rw> parameter.  In
...
この呼び出しはディレクトリーを削除できません。空のディレクトリーを削除するには L</rmdir> を、ディレクトリーを再帰的に削除するには
L</rm-rf> を使用します。
...
You can use L</vfs-uuid> to return the existing UUID of a filesystem.
...
I<This function is deprecated.> In new code, use the L</set-hv> call
instead.
...
To read the UUID on a filesystem, call L</vfs-uuid>.
...
case, the partition should be marked as "active" (L</part-set-bootable>) and
a Master Boot Record must be installed (eg. using L</pwrite-device>) on the
first sector of the whole disk.  The SYSLINUX package comes with some
