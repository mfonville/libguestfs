...
L<virt-resize(1)> and virt-sysprep can now use URIs to specify a remote
disk.
...
"Attach methods" are now known as "backends".  Compatibility with existing
code is preserved.
...
The febootstrap tool has now been renamed to "supermin".  Libguestfs can use
either, but now prefers "supermin".
...
virt-rescue.  In addition, C<rsync> can be used through the API for doing
incremental copies.
...
The huge C<src/guestfs.c> file is now split into smaller logical units.
...
The C<tests/extra> directory has gone.  The "extra tests" are now split into
separately runnable targets, such as C<make check-valgrind>.  Use C<make
