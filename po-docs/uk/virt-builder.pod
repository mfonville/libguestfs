...
this doesn't cache packages (the I<--install> option).
...
Встановити спосіб шифрування паролів: C<md5>, C<sha256> або C<sha512>.
...
See L</USERS AND PASSWORDS> below for the format of the C<SELECTOR> field,
...
virt-builder, look at the scripts in C<builder/website>
...
the amount of local caching that happens.  See L</ENVIRONMENT VARIABLES> and
L<curl(1)>.
...
I<--run> scripts are running out of memory.
